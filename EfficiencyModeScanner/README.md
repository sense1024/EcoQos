...
From Visual Studio: set **Application arguments** (e.g. `-v`, `-ecore`, or `-eq`) in project **Debug** properties.
...
Any argument other than `-v`, `-ecore`, or `-eq` (case-insensitive) is **invalid**: message on stderr, help text, exit code **1**.
...
Process names to **skip** when applying **`-ecore`** affinity. Matching is **case-insensitive** and uses the same convention as `System.Diagnostics.Process.ProcessName` (**no `.exe` suffix**).
...
Written/updated by **`-v`** and read by **`-ecore` / `-eq`**:

| Property | Type | Description |
|----------|------|-------------|
| `generatedAt` | ISO 8601 string | UTC timestamp when the file was written. |
| `processExclusions` | `string[]` | Process names to skip for `-ecore` / `-eq` (trimmed, distinct, sorted by app behavior). |
...
- **`-ecore` / `-eq`** do not use stored PIDs; they resolve current instances by name each run.
