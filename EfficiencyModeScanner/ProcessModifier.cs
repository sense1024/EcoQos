...
internal static class ProcessModifier
{
    internal static Task RunAsync(
        IReadOnlyList<string> targetNames,
        IReadOnlyList<string> processExclusions,
        uint openAccess,
        Func<nint, bool> applyAction,
        string description)
    {
        if (targetNames.Count == 0)
        {
            Console.WriteLine($"appsettings.json has no process names in \"{description}\".");
        ...
        Console.WriteLine($"Applying {description} for process names from JSON.");
        ...
        foreach (string processName in targetNames)
        {
            ...
            foreach (Process proc in running)
            {
                using (proc)
                {
                    ...
                    try
                    {
                        if (!applyAction(handle))
                        {
                            int err = Marshal.GetLastWin32Error();
                            Console.Error.WriteLine($"'{processName}' PID {pid}: {description} failed (error {err}).");
                            continue;
                        }

                        Console.WriteLine($"Set {description}: {processName} (PID={pid})");
                    }
                    finally
                    {
